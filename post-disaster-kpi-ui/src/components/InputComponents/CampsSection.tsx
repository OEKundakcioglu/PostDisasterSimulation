...
const explanations = {
    ...
    interArrivalData:
        "Inter-arrival data defines the timing between consecutive demand occurrences. Mean defines the demand frequency.",
};
...
const defaultDemand = (itemName: string, demandClass: DemandClass): CampDemand => {
    return {
        item: itemName,
        demandClass,
...
const buildDemandGroups = (demands: CampDemand[]) => {
    ...
    const emptyGroups: { key: string; item: string; internalIdx: number; externalIdx: number }[] = [];
...
const ensurePairForItem = (
    ...
    const hasInternal = camp.demands.some((d) => d.item === itemName && d.demandClass === "INTERNAL");
    const hasExternal = camp.demands.some((d) => d.item === itemName && d.demandClass === "EXTERNAL");
    ...
    setCamps((prev) => {
        ...
        const existingInternal = ds.find((d) => d.item === itemName && d.demandClass === "INTERNAL");
        const existingExternal = ds.find((d) => d.item === itemName && d.demandClass === "EXTERNAL");
        ...
        const makeFromBase = (demandClass: DemandClass) => {
            ...
            return {
                ...
                arrivalData: { ...b.arrivalData, distParameters: { ...b.arrivalData.distParameters } },
                leadTimeData: { ...b.leadTimeData, distParameters: { ...b.leadTimeData.distParameters } },
            } as CampDemand;
        };
                ...
                <Grid container spacing={2}>
                    <Grid item xs={12}>
                        <TextField
                            fullWidth
                            type="number"
                            label="Mean Inter-arrival Time (Minutes)"
                ...
                >
                    <Box>
                        <Box sx={{ display: "flex", alignItems: "center", gap: 1, mb: 2, opacity: 0.7 }}>
                            ...
                            <Typography variant="subtitle2" fontWeight={700} textTransform="uppercase" letterSpacing={1}>
                                Expected Demand
                            </Typography>
                        </Box>
                    ...
                    {(totalPerDay > 0) && (
                        <Box sx={{ mb: 3, height: 6, width: '100%', bgcolor: 'white', borderRadius: 4, overflow: 'hidden', display: 'flex' }}>
                            <Box sx={{ width: `${(internalPerDay/totalPerDay)*100}%`, bgcolor: 'primary.main' }} />
                            <Box sx={{ width: `${(externalPerDay/totalPerDay)*100}%`, bgcolor: 'secondary.main' }} />
                        </Box>
                    )}
                    ...
                    <Stack spacing={2}>
                        ...
                        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                            <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                                ...
                                <Typography variant="body2" sx={{ opacity: 0.8 }}>Internal</Typography>
                            </Box>
                        ...
                        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                            <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                                ...
                                <Typography variant="body2" sx={{ opacity: 0.8 }}>External</Typography>
                            </Box>
                            <Typography variant="subtitle2" fontWeight={700}>
...
const CampsSection: React.FC<Props> = ({ camps, setCamps, items }) => {
    const handleCampChange = (index: number, field: keyof Camp, value: string | boolean) => {
            ...
            {camps.map((camp, campIndex) => {
                ...
                return (
                    ...
                    >
                        <Grid container spacing={2}>
                            <Grid item xs={12}>
                                <NestedCollapsibleSection title="Basic Camp Information" level="tertiary">
                                    <Grid container spacing={2}>
                                        <Grid item xs={12} sm={6} md={4}>
                                            <TextField
                                                ...
                                                label="Name"
                                                value={camp.name}
                                                onChange={(e) => handleCampChange(campIndex, "name", e.target.value)}
                                            />
                                        </Grid>
                                        ...
                                        <Grid item xs={12} sm={6} md={4}>
                                            <TextField
                                                ...
                                                onChange={(e) =>
                                                    handleCampChange(campIndex, "initialInternalPopulation", e.target.value)
                                                }
                                            />
                                        </Grid>
                                        ...
                                        <Grid item xs={12} sm={6} md={4}>
                                            <TextField
                                                ...
                                                onChange={(e) =>
                                                    handleCampChange(campIndex, "initialExternalPopulation", e.target.value)
                                                }
                                            />
                                        </Grid>
                                    </Grid>
                            ...
                            {groups.map((g) => {
                                ...
                                const internalDemand = internalIdx >= 0 ? camp.demands[internalIdx] : undefined;
                                const externalDemand = externalIdx >= 0 ? camp.demands[externalIdx] : undefined;
                                ...
                                return (
                                    <Grid item xs={12} key={`demand-group-${g.key}`}>
                                                    ...
                                                    <IconButton
                                                        ...
                                                        onClick={(e) => {
                                                            ...
                                                            if (window.confirm("Are you sure you want to delete this demand?")) {
                                                                const newCamps = [...camps];
                                                                const newDemands = [...newCamps[campIndex].demands];

                                                                const removeIdxs = [internalIdx, externalIdx]
                                                                    .filter((x) => x >= 0)
                                                                    .sort((a, b) => b - a);

                                                                for (const idx of removeIdxs) newDemands.splice(idx, 1);
                                        ...
                                        >
                                            <Grid container spacing={2}>
                                                <Grid item xs={12}>
                                                    <NestedCollapsibleSection title="Basic Properties" level="tertiary">
                                                        <Grid container spacing={2}>
                                                            <Grid item xs={12} sm={6} md={4}>
                                                                {itemName ? (
                                                                    <TextField
                                                                    ...
                                                                    <TextField
                                                                        ...
                                                                        onChange={(e) => {
                                                                            ...
                                                                            if (internalIdx >= 0)
                                                                                ds[internalIdx] = { ...ds[internalIdx], item: newItem };
                                                                            if (externalIdx >= 0)
                                                                                ds[externalIdx] = { ...ds[externalIdx], item: newItem };
                                                                    ...
                                                                    >
                                                                        {items.length === 0 ? (
                                                                            <MenuItem disabled>No items available. Please add items first.</MenuItem>
                                                                        ) : (
                                                                            items
                                                                                .filter((itm) => itm.name)
                                                                                .sort((a, b) => a.name.localeCompare(b.name))
                                                                                .map((itm) => (
                                                                                    <MenuItem key={`item-${itm.name}`} value={itm.name}>
                                                                                        {itm.name}
                                                                                    </MenuItem>
                                                                                ))
                                                                        )}
                                                            ...
                                                            <Grid item xs={12} sm={6} md={4}>
                                                                <TextField
                                                                    ...
                                                                    onChange={(e) => {
                                                                        ...
                                                                        if (internalIdx >= 0)
                                                                            ds[internalIdx] = { ...ds[internalIdx], demandTimingType: val };
                                                                        if (externalIdx >= 0)
                                                                            ds[externalIdx] = { ...ds[externalIdx], demandTimingType: val };

                                                                        newCamps[campIndex].demands = ds;
                                                                        setCamps(newCamps);
                                                                    }}
                                                                >
                                                                    <MenuItem value="ONETIME">{formatLabel("ONETIME")}</MenuItem>
                                                                    <MenuItem value="RECURRING">{formatLabel("RECURRING")}</MenuItem>
                                                                </TextField>
                                                            </Grid>
                                                        </Grid>
                                                    </NestedCollapsibleSection>
                                                    ...
                                                    <NestedCollapsibleSection
                                                        title={
                                                            <Box sx={{ display: "flex", alignItems: "center" }}>
                                                                ...
                                                                <Tooltip title={explanations.leadTime} arrow placement="top">
                                                    ...
                                                    >
                                                        ...
                                                        <Grid container spacing={2}>
                                                            <Grid item xs={12} sm={6} md={4}>
                                                                ...
                                                                >
                                                                    <MenuItem value="EXPONENTIAL">{formatLabel("EXPONENTIAL")}</MenuItem>
                                                                    <MenuItem value="NORMAL">{formatLabel("NORMAL")}</MenuItem>
                                                                    <MenuItem value="UNIFORM">{formatLabel("UNIFORM")}</MenuItem>
                                                                    <MenuItem value="TRIANGULAR">{formatLabel("TRIANGULAR")}</MenuItem>
                                                                    <MenuItem value="FIXED">{formatLabel("FIXED")}</MenuItem>
                                                                </TextField>
                                                            </Grid>

                                                            {(master.leadTimeData.distributionType === "EXPONENTIAL" ||
                                                                master.leadTimeData.distributionType === "FIXED" ||
                                                                master.leadTimeData.distributionType === "EQUAL_SHARE") && (
                                                                <Grid item xs={12} sm={6} md={4}>
                                                                    <TextField
                                                                        fullWidth
                                                                        label="Mean (days)"
                                                                        value={master.leadTimeData.distParameters.mean || ""}
                                                                        onChange={(e) => {
                                                                            const val = e.target.value;
                                                                            const newCamps = [...camps];
                                                                            const ds = [...newCamps[campIndex].demands];
                                                                            ...
                                                                            const patch = (idx: number) => {
                                                                                ds[idx] = {
                                                                                    ...
                                                                                    leadTimeData: {
                                                                                        ...ds[idx].leadTimeData,
                                                                                        distParameters: {
                                                                                            ...ds[idx].leadTimeData.distParameters,
                                                                                            mean: val,
                                                                                        },
                                                                                    },
                                                                                };
                                                            ...
                                                            {master.leadTimeData.distributionType === "NORMAL" && (
                                                                <>
                                                                    <Grid item xs={12} sm={6} md={4}>
                                                                        <TextField
                                                                            fullWidth
                                                                    ...
                                                                    <Grid item xs={12} sm={6} md={4}>
                                                                        <TextField
                                                                            ...
                                                                            value={master.leadTimeData.distParameters.stdDev || ""}
                                                                            onChange={(e) => {
                                                                                const val = e.target.value;
                                                                                const newCamps = [...camps];
                                                                                const ds = [...newCamps[campIndex].demands];
                                                            ...
                                                            {master.leadTimeData.distributionType === "TRIANGULAR" && (
                                                                <>
                                                                    <Grid item xs={12} sm={6} md={4}>
                                                                        <TextField
                                                                            fullWidth
                                                                            label="Minimum (days)"
                                                                            value={master.leadTimeData.distParameters.min || ""}
                                                                            onChange={(e) => {
                                                                                const val = e.target.value;
                                                                                const newCamps = [...camps];
                                                                                const ds = [...newCamps[campIndex].demands];
                                                                    ...
                                                                    <Grid item xs={12} sm={6} md={4}>
                                                                        <TextField
                                                                            fullWidth
                                                                            label="Mode (days)"
                                                                            value={master.leadTimeData.distParameters.mode || ""}
                                                                            onChange={(e) => {
                                                                                const val = e.target.value;
                                                                                const newCamps = [...camps];
                                                                                const ds = [...newCamps[campIndex].demands];
                                                                    ...
                                                                    <Grid item xs={12} sm={6} md={4}>
                                                                        <TextField
                                                                            fullWidth
                                                                            label="Maximum (days)"
                                                                            value={master.leadTimeData.distParameters.max || ""}
                                                                            onChange={(e) => {
                                                                                const val = e.target.value;
                                                                                const newCamps = [...camps];
                                                                                const ds = [...newCamps[campIndex].demands];
                                                            ...
                                                            {master.leadTimeData.distributionType === "UNIFORM" && (
                                                                <>
                                                                    <Grid item xs={12} sm={6} md={4}>
                                                                        <TextField
                                                                            fullWidth
                                                ...
                                                <Grid item xs={12}>
                                                    <NestedCollapsibleSection
                                                        title={
                                                            <Box sx={{ display: "flex", alignItems: "center" }}>
                                                                Inter-arrival Data
                                                                <Tooltip title={explanations.interArrivalData} arrow placement="top">
                                                                    <IconButton size="small" sx={{ ml: 1 }}>
                                                                        <InfoIcon fontSize="small" />
                                                                    </IconButton>
                                                                </Tooltip>
