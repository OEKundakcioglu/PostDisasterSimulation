...
const InventoryPoliciesSection: React.FC<Props> = ({
  ...
  useEffect(() => {
    // Only update if there are no manual edits
    ...
    // Set buffer ratios and periodic counts for each camp and item
    camps.forEach((camp) => {
    ...
    // Set central buffer ratios and central periodic counts for each item
    items.forEach((item) => {
