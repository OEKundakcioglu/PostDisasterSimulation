...
const ItemsSection: React.FC<Props> = ({ items, setItems }) => {
  const handleItemChange = (
    ...
    subSubField?:
      | keyof NonNullable<Item["leadTimeData"]["distParameters"]>
      | keyof NonNullable<Item["durationData"]>["distParameters"]
  ) => {
    ...
    if (subField && subSubField) {
      ...
      if (
        ...
        const mainField = newItems[index][field] as any;
        if (mainField[subField]) {
          (mainField[subField] as any)[subSubField] = value;
        }
      }
    } else if (subField) {
